...
const NAV_SECTIONS: { title: string; items: { href: string; label: string; icon: string }[] }[] = [
  {
    ...
    items: [
      ...
      { href: ROUTES.dashboardBookings, label: 'Bookings', icon: 'line-md:calendar' },
      { href: ROUTES.dashboardInquiries, label: 'Requests', icon: 'line-md:bell' },
  ...
  },
  {
    ...
    items: [
      ...
      { href: ROUTES.dashboardFeatures, label: 'Suggest a feature', icon: 'line-md:lightbulb' },
    ],
  },
];

/**
 * The host Extranet chrome. Auth + role are resolved server-side in the layout
 * and passed in as props, so there is no client-side getUser() waterfall and no
 * dead-button hydration window.
 */
