...
// modal right where they clicked, instead of bouncing through a dead-end
// "sign in" page.
...
export function ExperienceContent({
  ...
  // Only days that match the host's weekly availability (and aren't in the
  // past) can be picked on the calendar.
  const availableDays = listing.available_days || [0, 1, 2, 3, 4, 5, 6];
  const today = useMemo(() => startOfDay(new Date()), []);
  const isCalendarDayDisabled = (date: Date) =>
    startOfDay(date) < today || !availableDays.includes(date.getDay());
  ...
  // The chosen session time, whichever source it came from — a real
  // availability slot if the host set any, otherwise a standard time choice.
  ...
  }, [selectedSlotId, slotsForSelectedDate, selectedStandardTime]);
  ...
  const inquiryDraft = useMemo<InquiryDraft>(
    () => ({
      ...
      hourlySlot: sessionTimeLabel,
      guests: Number(guests) || 1,
      availabilityStatus: selectedDateStr ? "AVAILABLE" : "NEEDS_CONFIRMATION",
    }),
    [selectedDateStr, sessionTimeLabel, guests]
  );

  // Deep link into the request-to-book flow, pre-filled with the chosen
  // date/time — mirrors the property page's reserve flow.
  const reserveHref =
    `/reserve/${listing.id}?category=experience` +
    `&checkIn=${selectedDateStr}` +
    `&startTime=${encodeURIComponent(sessionTimeLabel)}` +
    `&guests=${guests}`;
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {listing.experience_types?.slice(0, 3).map((type) => (
              <Badge key={type} variant="outline" className="border-cream text-merlot rounded-full py-0.5 px-3 font-semibold text-xs capitalize">
                {type}
              </Badge>
            ))}
          </div>
          <h1 className="text-3xl font-extrabold tracking-tight text-[#2b000a] sm:text-5xl leading-tight">
      ...
      <div className="grid grid-cols-1 gap-12 lg:grid-cols-[minmax(0,1fr)_400px] 2xl:grid-cols-[minmax(0,1fr)_460px] 2xl:gap-16">
        ...
        <div className="space-y-12">
          ...
          <div className="space-y-4">
            <h2 className="text-2xl font-extrabold text-[#2b000a]">What we will do</h2>
          ...
          {listing.house_rules && (
            <>
              <Separator className="bg-cream/60" />
              <div className="space-y-4">
                <h2 className="text-2xl font-extrabold text-[#2b000a]">What to bring / Special guidelines</h2>
                <div className="text-base leading-relaxed text-zinc-700 whitespace-pre-line font-medium text-[16px]">
                  {listing.house_rules}
                </div>
              </div>
        ...
        <div className="h-fit rounded-3xl border border-cream bg-white p-6 sm:p-8 shadow-md space-y-6 lg:sticky lg:top-28">
          <div className="space-y-1">
            <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-wider">TICKET PRICE</p>
            <div className="flex items-baseline gap-1">
              <span className="text-3xl font-extrabold text-[#2b000a]">
          ...
          <Separator className="bg-cream/50" />

          <div className="space-y-5">
            {/* 1. Date — a real calendar, not a text dropdown, so guests can
                see the host's weekly availability at a glance. */}
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-merlot uppercase tracking-wider block">
                Select Date
              </label>
              {dateChoices.length === 0 ? (
                <p className="rounded-xl border border-cream bg-zinc-50 px-3 py-2.5 text-sm font-semibold text-zinc-500">
                  No dates available
                </p>
              ) : (
                <Calendar
                  ...
                  className="mx-auto w-full bg-transparent p-0 [--cell-radius:999px]"
                />
              )}
            </div>
          ...
          <div className="space-y-3 font-semibold text-merlot">
            ...
            <div className="flex justify-between border-t border-cream/60 pt-3 text-base font-extrabold text-[#2b000a]">
              <span>Total Price</span>
              <span>{formatPrice(totalPrice, listing.currency || "KES")}</span>
            </div>
          </div>

          {/* Booking CTA — same request-to-book flow as the property page:
              primary action reserves the slot, secondary opens the chat. */}
          <div className="pt-2 space-y-2">
            {isOwnListing ? (
              <Link
                href={`/host/listings/${listing.id}/edit`}
                className="w-full flex h-12 items-center justify-center rounded-full bg-[#800020] font-bold text-white hover:bg-merlot transition shadow-sm text-sm uppercase tracking-wider"
              >
                Manage Experience
              </Link>
            ) : (
              <>
                ...
                <Button
                  ...
                  variant="outline"
                  className="w-full h-12 rounded-full border-cream font-bold text-merlot hover:bg-cream/30 text-sm uppercase tracking-wider"
                >
                  Message host
                </Button>
              </>
